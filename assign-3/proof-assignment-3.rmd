...
d. False: $\exists x \exists y(((x \ne y)^(x-4)^2=36)^(x-4)^2=36).$
    - This is false for the same reason that b was true. Only one natural number --
      ...
      false. However is the set $\mathbb{N}$, only 10 produces the desired result.
...
b. Every positive real number has a positive multiplicative inverse.
    - $\forall x \in \mathbb{R}^+ \exists y((x \cdot y = 1) ^ (x > 0))$; or
...
b. Fasle: $\wp (A) \cup \wp (B)=\wp (A) \cup B$
    - Suppose $A=\{1,2\} \textrm{ , } B=\{2,3\}, \textrm{ and } A \cup B =
      \{1, 2, 3\}. \textrm{ Then } \wp (A)=\{\emptyset, \{1\},\{2\}, \{1,2\}\}
      \textrm{ and } \wp (B)=\{\emptyset, \{2\}, \{3\}, \{2,3\}\}.
      \textrm{ Therefore } \wp (A) \cup \wp (B)=\{\emptyset, \{1\}, \{2\},
      \{3\}, \{1, 2\}, \{2, 3\}\}. \textrm{ Whereas } \wp (A \cup B)=\{\emptyset,
      \{1\}, \{2\}, \{3\}, \{1, 2\}, \{2, 3\},\\ \{1, 3\}, \{1, 2, 3\}\}.
      \textrm{ Clearly, } \wp (A \cup B)$ has subsets {1, 3} and {1, 2, 3} that are not
      present in $\wp (A) \cup \wp (B). \textrm{ Therefore } \wp (A) \cup \wp (B)
      \ne\wp (A \cup B).$ 
